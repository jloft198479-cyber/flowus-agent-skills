 ...
 * @throws {Error} 当 status >= 400 时抛出描述性错误（含 request_id 便于排查）
 */
function _assertOk(resp) {
  if (resp.status >= 400) {
    ...
    throw new Error(code ? `${code} (HTTP_${resp.status}): ${errMsg}${requestId}` : `HTTP_${resp.status}: ${errMsg}${requestId}`);
  }
}
 ...
 * @param {object} [options.headers] - 额外请求头（如 Idempotency-Key 用于 POST /v2/pages 幂等创建）
 * @returns {Promise<object>} 响应体
 *
 * @example
 * const results = await rest.post('/search', { query: '关键词', page_size: 20 });
 * // 幂等创建页面（相同 Idempotency-Key + 相同请求体 → 返回相同结果）
 * const page = await rest.post('/pages', body, { headers: { 'Idempotency-Key': uuid } });
 */
...
async function uploadFile(params) {
  ...
  // 自动检测 MIME 类型（官方白名单：图片/文档/文本/压缩包/音视频/其他）
  const MIME_MAP = {
    // 图片
  ...
  // 官方文档建议：不要手动设置 Content-Length，让 HTTP 客户端按实际字节数自动计算
